...
function passStation(): string {
  return `LEFT JOIN LATERAL (SELECT i."id" AS "inspectionId", i."attempt_number" AS "attemptNumber", i."completed_at" AS "completedAt", station."id" AS "stationId", station."code" AS "stationCode", station."name_kh" AS "stationNameKh", station."name_en" AS "stationNameEn" FROM "inspections" i INNER JOIN "appointments" appointment ON appointment."id" = i."appointment_id" AND appointment."daily_capacity_id" IS NOT NULL INNER JOIN "inspection_station_daily_capacities" capacity ON capacity."id" = appointment."daily_capacity_id" INNER JOIN "inspection_stations" station ON station."id" = capacity."station_id" WHERE i."application_id" = application."id" AND i."status" = 'COMPLETED'::"public"."inspection_status" AND i."result" = 'PASS'::"public"."inspection_result") passed ON true`;
}
