...
- `docs/api/02-rest-api-contracts.md` (75 REST endpoints, response/request contracts, errors, and transactions)
- `docs/database/mpwt_vehicle_inspection_full_schema.dbml`
- `docs/database/mpwt_vehicle_inspection_constraints.sql`
- the 15 TypeORM entities, 18 enums, and two approved migrations.
...
| --- | --- | --- |
| Application bootstrap | `src/main.ts` creates `AppModule` and listens on `PORT` (default `3000`). No API prefix, global pipe, or global filter is configured yet. | Add those foundation concerns centrally in a later implementation phase; do not duplicate them by controller. |
...
| Data layer | `database.entities.ts` centrally lists all 15 entities. `DatabaseModule` uses `forRootAsync`; shared options use a runtime-relative migration glob and have `synchronize`, `dropSchema`, and `migrationsRun` disabled. | Keep TypeORM, PostgreSQL, migration discipline, and explicit entity ownership. |
| Migrations | `1785378252215-InitialSchema.ts` and `1785380837520-AddAdvancedDatabaseConstraints.ts` are present. | Treat the approved constraints as final integrity protections; do not compensate for them with schema redesign. |
...
| Tests | Jest unit configuration is rooted at `src`; an e2e Jest config exists. The current tests are only Nest starter tests. | Add focused domain/service, authorization, integration, and e2e tests in phases rather than broad tests first. |
...
src/
  ...
  auth/            verification-code entity; auth shells
  common/          empty shared module shell
...
- Keep the approved stack: NestJS, TypeORM, PostgreSQL, REST, and access-token-only JWT authentication.
...
| --- | --- | --- |
| `AuthModule` | Register, verify, resend, login, reset, JWT issuance/validation support, admin bootstrap. | Retain. It owns verification codes and authentication orchestration, not profiles or broad authorization policy. |
...
src/
  ...
  auth/
    controllers/
    dto/requests/
    services/             # AuthService, verification/password/JWT collaborators as needed
    mappers/
    entities/
    enums/
  users/
...
| Concern | Proposed owner/location | Practical rule |
| --- | --- | --- |
...
| Global validation | `main.ts` later: one `ValidationPipe` | Enable whitelist, forbid unknown input, transformation, and contract-safe validation errors. DTOs remain explicit per write/query route. |
| Response envelopes | `common/http/` | Use explicit helpers/types for `{ data }`, `{ data, meta }`; do not use an interceptor that serializes entities directly. |
| Errors | `common/errors/` | A small `DomainException` carries one approved code, safe message, optional validation details, and intended HTTP status. One global filter produces `ApiErrorResponse`. |
...
| Pagination/filter/sort | `common/pagination/` plus feature query DTOs | Base page fields are shared; every feature provides its own explicit filter and sort allowlist. |
...
| Request context | `common/request-context/` | Extract safe client IP/user agent for audit writes; never put credentials/tokens in audit data. |
...
`AuthService` remains the public orchestration owner. It should coordinate user/profile creation with `UsersService` through a narrow exported method, verification-code persistence, password/code hashing, JWT creation, and safe responses. It must not expose hashes or verification records.
...
| Verification/resend | Endpoint selects `REGISTER_ACCOUNT` server-side; lock/check the latest eligible code, expiry, single-use state, and attempt count. Verification sets the matching verified timestamp and activates the citizen. |
| Login | Resolve one normalized phone/email identifier, compare password hash, require `ACTIVE`, update `lastLoginAt`, issue access token. Do not reveal account existence beyond approved errors. |
| Password reset | Request always returns an enumeration-safe response. Confirmation selects `RESET_PASSWORD` server-side, consumes the eligible code, and replaces password hash atomically. |
| JWT | An internal Auth JWT/token collaborator creates and verifies access tokens. The JWT guard reads identity claims; `ActiveUserGuard` reloads/checks current user status after token validation. |
...
The current dependency list contains neither a dedicated Nest JWT package nor a password-hashing package. Before implementing Phase 2, approve a minimal JWT and password-hash library choice, add only those required packages, and document their configuration. Do not add Passport, refresh-token storage, logout persistence, OAuth, or a generic identity platform by default.
...
| Transactional endpoint/operation (24) | Lock/check pattern and atomic effects |
| --- | --- |
| 1. Register | Check normalized phone/email unique constraints; create user/profile/hashed registration code. |
| 2. Verify account | Lock eligible registration code/user; consume code, update verified time/status, audit as required. |
| 3. Reset-password confirm | Lock eligible reset code/user; replace hash and consume code. |
| 4. Admin user status | Lock user; allow only `ACTIVE`/`DISABLED`; update plus audit. |
| 5. Submit application | Lock/read owned vehicle; rely on `uq_active_application_per_vehicle`; create application/snapshots/three documents/timeline/notification/audit. |
| 6. Resubmit correction | Lock application/current documents; flip prior `isCurrent`, insert versions, rely on `uq_current_document_per_type`, create correction timeline/audit only. |
| 7. Citizen application cancel | Lock application/payment/inspection and any scheduled appointment; cancel appointment and release capacity if present, then application effects. |
| 8. Admin application cancel | Same cancellation coordination; when payment is confirmed, retain it unchanged without refund/reversal. |
| 9. Start review | Lock application; validate `SUBMITTED`; set review fields/timeline/notification/audit. |
| 10. Request correction | Lock application/documents; validate review state/reason; transition with approved effects. |
| 11. Mark ready | Lock application/current docs; validate all three approved; transition with approved effects. |
| 12. Review document | Lock current document; validate status/rejection reason; update review data and audit. |
| 13. Book appointment | Lock slot or use equivalent safe protection; verify `OPEN`, count `SCHEDULED`, enforce capacity and `uq_scheduled_appointment_per_application`; create appointment/payment-if-absent/timeline/notification/audit. |
| 14. Citizen appointment cancel | Lock appointment; verify ownership, scheduled status, and start time; cancel with effects. |
| 15. Reschedule appointment | Lock current appointment and target slot; cancel old and create new in one transaction, never update `slotId`. |
| 16. Admin appointment cancel | Lock scheduled appointment; require operational reason; cancel with effects. |
| 17. Mark no-show | Lock eligible scheduled appointment; confirm no inspection; mark `NO_SHOW` with effects. |
| 18. Confirm payment | Lock `PENDING`/`REJECTED` pay-at-station payment; ensure receipt uniqueness; preserve rejection fields; confirm/timeline/notification/audit. |
| 19. Reject payment | Lock `PENDING` pay-at-station payment; set rejection fields and audit only, with no payment-failed timeline/notification. |
| 20. Create inspection | Lock/read appointment/application/payment; validate composite consistency, confirmed payment, and one inspection per appointment. |
| 21. Complete inspection | Lock pending inspection/appointment/application; complete appointment in same transaction; PASS creates/retains sticker, FAIL terminalizes application; write approved effects. |
| 22. Mark sticker ready | Lock eligible sticker/application; record ready state/actor/time and approved effects. |
| 23. Issue sticker | Lock sticker/application/inspection; require completed PASS; issue sticker and complete application atomically. |
| 24. Announce | Select active citizens and insert one `IN_APP`/`SYSTEM_ANNOUNCEMENT` notification per recipient plus audit in one transaction. |
...
Each feature mapper converts entities/projections to Task 4B response models. Mapper input is explicitly loaded with only relations needed by the endpoint. Examples: `UserMapper` returns safe `UserSummary`; `CitizenProfileMapper` uses `nationalIdNumber` and an authorized `profileImageUrl`/availability rather than `profileImageKey`; `VehicleMapper` uses `linkedCitizenId`; application mapper exposes `referenceNumber`; auth mapper exposes access token only in approved login/verification responses.
...
- **REST e2e tests:** Nest app with test configuration, global pipe/filter/guards enabled, and focused essential flows. Preserve the existing starter tests only until feature coverage supersedes them.

Priority scenarios are authentication/verification; cross-citizen access denial; all eight transitions; document replacement history/current uniqueness; slot-capacity concurrency; one scheduled appointment; `REJECTED` to `CONFIRMED` payment retry; confirmed-payment cancellation retention; inspection-driven appointment completion; failed-inspection terminal behavior; sticker issuance/application completion; and atomic timeline/notification/audit results. Do not require a test for every trivial mapper.

## 15. Implementation sequence

| Phase | Modules and essential endpoints | Prerequisites / principal tests / done definition |
| --- | --- | --- |
| 1. Common foundation | Config, Database, Common; API prefix, pipe, envelopes, error filter, request context, transaction convention. | Existing build passes; configuration validation/error/pagination tests; done when no feature endpoint bypasses conventions. |
| 2. Authentication and users | Auth, Users; six public auth routes, current user, citizen-profile patch, admin user reads/status; controlled bootstrap. | Approve minimal auth/hash dependencies and new environment settings; auth/disabled/enumeration/verificationIdentifier tests; done when protected identity is reliable. |
...
Do not attempt all 75 endpoints in one implementation task. A phase is complete only when its endpoint contract, authorization, mapper, error behavior, focused tests, and relevant transaction behavior pass.
...
Avoid cycles by making the module that owns the user action orchestrate collaborators, exporting only narrow services needed by another module, passing transaction-aware side-effect services downward, and sharing only types/errors in `common` when genuinely shared. `StickersModule` must not import `InspectionsModule`: inspection completion may call an exported narrow sticker-workflow service to create or retain the `NOT_READY` sticker, while sticker readiness/issuance validates a completed `PASS` inspection through a narrow read-only query or repository access without importing the whole inspections module. The orchestration owner determines dependency direction. `forwardRef` is a last-resort signal to redesign the dependency, not a default solution.

## 17. Expected environment configuration

No environment file changes are part of this task. The existing validated variables are `NODE_ENV`, `PORT`, `DB_HOST`, `DB_PORT`, `DB_USERNAME`, `DB_PASSWORD`, `DB_NAME`, and `DB_LOGGING`. Before the relevant implementation phases, validation and `.env.example` should be extended with safe placeholders such as:
...
| JWT | `JWT_SECRET=<replace-me>`, `JWT_EXPIRES_IN=<approved-duration>` |
| Password hashing | `PASSWORD_HASH_*` settings appropriate to the approved selected library |
...
`.env` remains ignored and contains real secrets only locally/deployed. `.env.example` contains non-secret placeholders; Docker/pgAdmin development values are not production credentials.
...
Task 5A does not implement controllers, services, DTO classes, guards, storage, Swagger decorators, OpenAPI YAML, entities, migrations, constraints, frontend code, package installation, or environment changes. It does not add endpoints or alter approved authentication, workflow, payment, refund, notification, or retention rules. It does not stage or commit files.

## 20. Decisions and readiness

There are no blocking architecture decisions for this documentation plan. Non-blocking items already identified by Task 4A/4B remain implementation constants: number formats, file MIME/size limits, password policy, JWT lifetime, storage provider/key format, notification wording, audit redaction/retention details, exceptional administrative correction procedure, and the local-development verification-code exposure choice.

This plan is ready to guide feature implementation after the minimal auth/hash dependency choice and the corresponding safe environment placeholders are approved in their implementation phase. It intentionally stops before Task 5B or any code work.
