...
| --- | --- | --- |
| Application bootstrap | `src/main.ts` creates `AppModule`, applies the `/api` global prefix, global `ValidationPipe`, and global exception filter through the shared API configuration, then listens on `PORT` (default `3000`). The unused Nest starter root route has been removed. | Retain these foundation concerns centrally; do not duplicate them by controller. |
...
| --- | --- |
| Application | `NODE_ENV`, `PORT`, `API_PREFIX=/api` |
